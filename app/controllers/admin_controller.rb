class AdminController < ApplicationController
  ...
  def index
    @order_unfullfiled = Order.where(fulfilled: false).order(created_at: :desc).take(5)
    @quick_stats ={
    ...
    if @revenue_by_day.count < 7
      ...
      ordered_days_with_current_last = days_of_week[next_day_index..-1] + days_of_week[0..current_day_index]
